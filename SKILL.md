...
- A refreshed or reopened browser may recover the existing SSH session through `GET /api/v1/sessions/recover` without requiring Agent authorization. Session stores only a hash of the high-entropy recovery token, binds it to the specific session and confirmed SSH fingerprint, and sends the token only in a host-only `HttpOnly; SameSite=Strict` Cookie with a rolling 30-day lifetime. Explicit disconnect clears the Cookie; a missing SSH session or restarted Session process makes it useless. This recovery never retrieves or persists an SSH password.
- `GET /api/v1/agent/session` remains a scoped Agent discovery fallback, not the browser's primary recovery mechanism.
...
- Expect Session to close a connection only after an explicit user disconnect, Session shutdown, or the SSH transport declaring the connection lost. Node ssh2 and Python AsyncSSH report the configured keepalive failure count; Paramiko reports transport-level detection without claiming a fixed failure threshold. Do not promise automatic reconnection after a real transport loss because connection passwords are never persisted.
