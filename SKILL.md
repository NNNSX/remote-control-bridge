...
- Treat SOCKS support in Python tooling as a capability to verify, not an assumption. Before retrying a proxied `pip` command, check the remote interpreter with `python -c "import socks; print(socks.__version__)"` (or the equivalent interpreter selected by the task). If it fails with `ModuleNotFoundError` or pip reports that SOCKS support is unavailable, do not keep retrying the same command: download a compatible `PySocks` wheel on the local machine, upload it through the Agent file API/SFTP to the active remote directory, install it with `python -m pip install <uploaded-wheel>`, then retry the original command with `proxy: true`.
- Prefer the exact remote Python version and platform when selecting the wheel. If the remote host has no usable package index even after PySocks is installed, resolve/download all required wheels locally and upload them, then use `python -m pip install --no-index --find-links <wheel-directory> ...`. Keep this as an on-demand compatibility step; do not install PySocks or modify the remote host unless a proxied Python command actually needs it.
- Before downloading dependencies, run a small network preflight with the same tool and interpreter that will do the real work. Use `curl --socks5-hostname` or an equivalent HTTPS request to distinguish SSH/proxy reachability from package-manager compatibility; keep `socks5h` so DNS resolution happens through the remote side. Never disable TLS certificate verification just to make a proxied request pass.
- Do not assume every client understands a SOCKS URL from `ALL_PROXY`: `curl` and modern Git-over-HTTPS usually do; Python `pip`/classic `conda` commonly need PySocks; `wget`, `npm`, and `apt` often expect an HTTP proxy or tool-specific support; Docker image pulls are performed by the daemon and do not inherit a command's temporary environment. Git over SSH also does not automatically use this proxy. For unsupported clients, prefer a local download plus SFTP upload, or an explicitly supported HTTP proxy, rather than changing persistent remote configuration.
- When a proxied command fails, classify it before retrying: (1) transport/DNS failure, (2) client lacks SOCKS support, (3) certificate/clock/CA failure, or (4) package index/dependency resolution failure. Apply only the matching fix, preserve the original error, and avoid repeated retries with unchanged settings.
